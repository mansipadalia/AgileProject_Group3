			...
			if (c.getProperty(PropertyType.children) != null) {
				...
				if (ChildrenIds.length >= 1) {
					for (int i = 0; i < ChildrenIds.length; i++) {
						...
						Predicate<Record> byHId = x -> x.getProperty(PropertyType.id).getValue().equals(husbandID);
						List<Record> resH = p.getIndividualList().stream().filter(byHId)
								.collect(Collectors.<Record>toList());
						...
						if (birthDateH != null && bdate_C != null
							...
							error.setMessage("Parent (" + resH.get(0).getProperty(PropertyType.id).getValue()
									+ ") (Birth Date: " + birthDateH + ") is older than his child ("
									+ resC.get(0).getProperty(PropertyType.id).getValue() + ") (Birth Date: " + bdate_C
									+ ").");
							errors.add(error);
						}

						Predicate<Record> byWId = x -> x.getProperty(PropertyType.id).getValue().equals(wifeID);
						List<Record> resW = p.getIndividualList().stream().filter(byWId)
								.collect(Collectors.<Record>toList());

						LocalDate birthDateW = wifeID == null ? null
								: resW.get(0).getProperty(PropertyType.birthday) != null
						...
						if (birthDateW != null && bdate_C != null
							...
							error.setLineNumber(resC.get(0).getProperty(PropertyType.birthday).getLineNumber());
							error.setId((String) c.getProperty(PropertyType.id).getValue());
							error.setMessage("Parent (" + resW.get(0).getProperty(PropertyType.id).getValue()
									+ ") (Birth Date: " + birthDateW + ") is older than her child ("
									+ resC.get(0).getProperty(PropertyType.id).getValue() + ") (Birth Date: " + bdate_C
									+ ").");
							errors.add(error);
						}
